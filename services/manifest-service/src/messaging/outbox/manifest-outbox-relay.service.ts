...
@Injectable()
export class ManifestOutboxRelayService
  implements OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(ManifestOutboxRelayService.name);
  ...
  private async flushPending(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Manifest outbox relay failed while publishing events: ${this.toErrorMessage(error)}`,
      );
    } finally {
