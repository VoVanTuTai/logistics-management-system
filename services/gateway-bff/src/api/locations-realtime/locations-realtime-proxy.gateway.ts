...
@Injectable()
export class LocationsRealtimeProxyGateway {
  private readonly logger = new Logger(LocationsRealtimeProxyGateway.name);
  ...
  registerHttpServer(server: Server): void {
    ...
    this.logger.log('Locations realtime WebSocket proxy is listening on /ws/locations');
  }
  ...
  private async handleUpgrade(
    ...
    if (requestUrl.pathname !== '/ws/locations') {
      return;
    }
    ...
    proxyRequest.on('response', (response) => {
      this.reject(
        ...
        `Location realtime upstream did not upgrade: ${response.statusCode ?? 502}`,
      );
      response.resume();
    });

    proxyRequest.on('error', (error) => {
      this.logger.warn(`Location realtime proxy failed: ${toErrorMessage(error)}`);
      this.reject(socket, 502, 'Location realtime upstream unavailable.');
    });
  ...
  private buildTargetUrl(requestUrl: URL): URL {
    const scanBaseUrl = this.serviceRegistryClient.resolveServiceUrl('scan');
    const targetUrl = new URL('/ws/locations', ensureTrailingSlash(scanBaseUrl));
