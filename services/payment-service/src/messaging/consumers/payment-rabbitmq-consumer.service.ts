...
import type { PaymentConsumerEnvelope } from './payment-events.consumer';
import { PaymentEventsConsumer } from './payment-events.consumer';

const RETRY_ROUTING_KEY = 'payment.retry';

@Injectable()
export class PaymentRabbitmqConsumerService
  implements OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(PaymentRabbitmqConsumerService.name);
  ...
  private async connectAndConsume(): Promise<void> {
    ...
    try {
      ...
      await channel.consume(
        this.paymentEventsConsumer.queueName,
        (message) => {
      ...
      this.logger.log(
        `Payment consumer is listening queue "${this.paymentEventsConsumer.queueName}".`,
      );
    } catch (error) {
      this.logger.error(
  ...
  private async configureTopology(channel: Channel): Promise<void> {
    const mainQueue = this.paymentEventsConsumer.queueName;
    const retryQueue10s = this.paymentEventsConsumer.retryQueues[0];
    const retryQueue1m = this.paymentEventsConsumer.retryQueues[1];
    const deadLetterQueue = this.paymentEventsConsumer.deadLetterQueue;
    ...
    for (const pattern of this.paymentEventsConsumer.routingPatterns) {
  ...
  private async handleMessage(message: ConsumeMessage | null): Promise<void> {
    ...
    if (!event) {
      this.logger.warn('Skipping malformed payment event payload.');
      channel.ack(message);
      return;
    }

    try {
      await this.paymentEventsConsumer.handle(event);
      channel.ack(message);
    } catch (error) {
  ...
  private moveToRetryOrDlq(
    ...
    const targetQueue =
      retryCount === 0
        ? this.paymentEventsConsumer.retryQueues[0]
        : retryCount === 1
          ? this.paymentEventsConsumer.retryQueues[1]
          : this.paymentEventsConsumer.deadLetterQueue;
    ...
    channel.ack(message);
    this.logger.warn(
      `Payment event failed, moved to "${targetQueue}" (retry=${retryCount}).`,
    );
  }

  private parseEventEnvelope(content: Buffer): PaymentConsumerEnvelope | null {
    let parsed: unknown;
    ...
    if (!eventType || !this.paymentEventsConsumer.routingPatterns.includes(eventType)) {
      return null;
    }
    ...
    return {
      event_type: eventType as PaymentConsumerEnvelope['event_type'],
