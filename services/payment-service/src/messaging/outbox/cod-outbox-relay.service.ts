...
@Injectable()
export class CodOutboxRelayService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(CodOutboxRelayService.name);
  ...
  private async flushPending(): Promise<void> {
    ...
    try {
      const pendingEvents = await this.outboxEventRepository.listPending(
        this.batchSize,
      );
      ...
      this.logger.error(
        `COD outbox relay failed while publishing events: ${this.toErrorMessage(error)}`,
      );
    } finally {
