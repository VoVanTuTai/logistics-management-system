...
import './styles.css';
import { PROVINCE_CITY_OPTIONS } from './location-options';
import {
  asRecord,
...
function MerchantApp(): React.JSX.Element {
  ...
  const navItems: Array<{ id: ViewId; label: string }> = [
    ...
    { id: 'tracking', label: 'Tra cuu van don' },
  ...
  async function submitCreateShipment(withPickup: boolean): Promise<void> {
    if (!session) return;
    if (!createForm.receiverRegion.trim()) {
      setCreateError('Vui long chon tinh / thanh pho nguoi nhan.');
      return;
    }

    if (!createForm.receiverAddress.trim()) {
      setCreateError('Vui long nhap dia chi chi tiet nguoi nhan.');
      return;
    }
      ...
      <div className="login-shell">
        <div className="login-card grid">
          ...
          <form className="grid" onSubmit={handleLogin}>
            <input className="input" value={loginUsername} onChange={(e) => setLoginUsername(e.target.value)} placeholder="Tai khoan" />
            <input className="input" type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="Mat khau" />
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="btn-row"><button className="btn btn-secondary" onClick={() => void refreshAllData(session.accessToken)} disabled={dataLoading}>{dataLoading ? 'Dang tai lai...' : 'Tai lai'}</button><button className="btn btn-danger" onClick={() => void handleLogout()}>Logout</button></div>
      </aside>
