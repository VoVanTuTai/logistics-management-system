...
import { getPickupScannerAdapter } from '../../features/scan/pickup.scanner.adapter';
...
export function PickupScanScreen({ route }: Props): React.JSX.Element {
  ...
  const handleScanQrOrBarcode = async () => {
    setScannerError(null);

    try {
      const scanResult = await getPickupScannerAdapter().scanShipmentCode();
      if (!scanResult) {
        return;
      }
