 ...
 * Dành cho Ops Command Center & HQ Network Geofencing
 * Bao gồm bộ dữ liệu GeoJSON chuẩn quốc gia 483 phường/xã
 */
...
export const VIETNAM_NATIONAL_BOUNDARY: BoundaryItem = {
  ...
  polygon: [
    ...
    [11.6000, 105.9500], // Tân Biên, Tây Ninh
    ...
    [14.7000, 107.5500], // Bờ Y, Ngọc Hồi, Kon Tum (Ngã ba Đông Dương)
    [15.5500, 107.4500], // Nam Giang, Quảng Nam
    [16.1500, 107.2500], // A Lưới, Thừa Thiên Huế
    [16.6500, 106.6500], // Cửa khẩu Lao Bảo, Quảng Trị
    [17.4500, 106.0500], // Cửa khẩu Cha Lo, Quảng Bình
    [18.3800, 105.2000], // Cửa khẩu Cầu Treo, Hà Tĩnh
    [19.2500, 104.2500], // Kỳ Sơn, Nghệ An
    [20.2500, 104.6000], // Cửa khẩu Na Mèo, Thanh Hóa
    [20.9500, 103.5500], // Sông Mã, Sơn La
    [21.6500, 103.0000], // Mường Nhé / Điện Biên Đông
    [22.4000, 102.1500], // A Pa Chải (Cực Tây Việt Nam)
    [22.6500, 102.8000], // Mường Tè, Lai Châu
    [22.7500, 103.4500], // Phong Thổ, Lai Châu
    [22.6000, 103.9500], // Bát Xát / Lào Cai
    [22.9500, 104.6500], // Hoàng Su Phì, Hà Giang
    [23.3927, 105.3238], // Khép kín tại Cực Bắc Lũng Cú
  ],
  islandPolygons: [
...
export function findWardForCoordinate(
  ...
  // 2. Fallback: EXPANDED_WARD_HUBS cũ
  ...
  // 3. Dung sai tiếp giáp đường biên
...
export function findRegionForCoordinate(lat: number, lng: number): BoundaryItem | null {
  ...
  if (lat >= 19.8) {
    return VIETNAM_REGION_BOUNDARIES[0];
  } else if (lat >= 11.5) {
    return VIETNAM_REGION_BOUNDARIES[1];
  } else {
    return VIETNAM_REGION_BOUNDARIES[2];
  }
}
